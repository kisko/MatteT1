...
import { ArrowLeft, BrainCircuit, Check, CheckCircle2, ChevronDown, Layers3, Pause, Play, RotateCcw, Sparkles } from 'lucide-react';
import { MathView } from '../MathView.js';

interface ExperimentalLabViewProps {
  onBack: () => void;
}
...
export const ExperimentalLabView: React.FC<ExperimentalLabViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-8 lg:px-8">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <button onClick={onBack} className="inline-flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2 text-xs font-semibold text-slate-300 transition-colors hover:bg-slate-700 hover:text-white">
          <ArrowLeft className="h-4 w-4" /> Tilbake
        </button>
        <span className="rounded-full border border-cyan-500/30 bg-cyan-950/40 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-cyan-300">Eksperimentell modul</span>
      </div>

      <div className="mb-6 max-w-3xl">
        <p className="mb-2 text-sm font-bold uppercase tracking-[0.18em] text-cyan-400">Matematisk laboratorium</p>
        <h1 className="text-3xl font-black tracking-tight text-white sm:text-5xl">Se hva funksjonen gjør.</h1>
