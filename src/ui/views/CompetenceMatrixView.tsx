    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-white sm:text-3xl">Kompetansemålmatrise</h1>
          <p className="mt-1 text-sm text-slate-400">Velg et mål for presis trening, eller åpne hele treningsmodulen.</p>
        </div>
      ...
      <div className="space-y-5">
        {visibleTopics.map((definition) => {
          ...
          return (
            <section key={definition.topic} className="border-y border-slate-800 py-5 first:border-t-0" aria-labelledby={`topic-${definition.topic}`}>
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h2 id={`topic-${definition.topic}`} className="text-xl font-bold text-white">{definition.title}</h2>
                  <p className="mt-1 text-sm text-slate-400">{definition.description}</p>
                </div>
                <button onClick={() => onOpenModule(definition.topic)} className="inline-flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm font-semibold text-slate-200 hover:border-indigo-400 hover:text-white">
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                {definition.goals.map((goal) => {
                  ...
                  return (
                    <article key={goal.id} className="flex min-h-48 flex-col border border-slate-800 bg-slate-900/50 p-4">
                      ...
                      <h3 className="mt-2 font-bold text-slate-100">{goal.title}</h3>
                      <p className="mt-2 text-xs leading-relaxed text-slate-400">{goal.description}</p>
                      <div className="mt-auto pt-4">
                        <div className="mb-3 flex items-center justify-between text-[11px] text-slate-500"><span>{availableTasks}/{goal.targetTasks} oppgaver</span><span>{attempted} forsøk</span></div>
                        <button onClick={() => onStartGoal(definition.topic, goal.taskLabels)} className="inline-flex items-center gap-1 text-sm font-semibold text-indigo-300 hover:text-indigo-200">
